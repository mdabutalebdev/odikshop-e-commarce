<x-layout title="আমার অ্যাকাউন্ট">
    <div class="container-x py-6">
        <div class="flex items-center justify-between mb-5">
            <h1 class="text-2xl font-extrabold">আমার অ্যাকাউন্ট</h1>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="h-10 px-4 rounded-lg border border-line bg-white text-sm font-semibold hover:bg-canvas"><i class="fa-solid fa-right-from-bracket mr-1"></i>লগআউট</button>
            </form>
        </div>
        ...
        <div class="grid lg:grid-cols-[320px_1fr] gap-6">
            ...
            <div class="bg-white rounded-xl border border-line p-5 h-fit">
                ...
                <form method="POST" action="{{ route('account.update') }}" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs font-medium mb-1 text-muted">নাম</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium mb-1 text-muted">ফোন</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium mb-1 text-muted">শহর</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium mb-1 text-muted">ঠিকানা</label>
                    ...
                    <button class="btn-brand w-full h-10 text-sm">প্রোফাইল আপডেট</button>
                </form>
            </div>

            {{-- Orders --}}
            <div class="bg-white rounded-xl border border-line p-5">
                <h2 class="font-bold mb-4 flex items-center gap-2"><i class="fa-solid fa-bag-shopping text-brand"></i>আমার অর্ডারসমূহ</h2>
                @if($orders->isEmpty())
                        ...
                        @foreach($orders as $order)
                            <div class="border border-line rounded-lg p-3 flex items-center justify-between flex-wrap gap-2">
                                <div>
                                    ...
                                    <div class="text-xs text-muted">{{ $order->created_at->format('d M Y') }} · {{ $order->items_count ?? $order->items->count() }} টি পণ্য</div>
                                </div>
